...
mask:		.word	0, 0, 0
		.word   0, 1, 0
		.word	0, 0, 0  # macierz 3 na 3
mask_sum:	.word	1

#file_name:	.space 100 	#nazwa pliku 
file_name:	.asciiz	"choinka.bmp" 
hello:		.asciiz	"filtr dolnoprzepustowy - podaj nazwe pliku\n"
mask_values:	.asciiz "podaj współczynniki filtru \n"
...
.macro store_pix(%pix, %x)
	sll	%pix, %pix,8
	usw	%pix, buf_out($s4)
	addiu	$s4, $s4, 3 	#bo pixel to 4 bajty
	bne	%x, $s5, save_buf_to_file 
	
add_padding:
	rem	$t3, $s4, 4  #modulo 4
	add	$s4, $s4, $t3
